...
if brand_inp == 'TOYOTA':
    ...
    model_inp = col1.selectbox('Enter the model for Misubishi', options=find_model('Daihatsu'))
    model = model_dic[model_inp] 

elif brand_inp == 'Honda':   
    model_inp = col1.selectbox('Enter the model Renault', options=find_model('Honda'))
    model = model_dic[model_inp]

elif brand_inp == 'Isuzu':
    model_inp = col1.selectbox('Enter the model for Toyota', options=find_model('Isuzu'))
    model = model_dic[model_inp] 

elif brand_inp == 'Lexus':
    model_inp = col1.selectbox('Enter the model for BWM', options=find_model('Lexus'))
    model = model_dic[model_inp] 
    
elif brand_inp == 'Mazda':
    model_inp = col1.selectbox('Enter the model for Mercedes Benz', options=find_model('Mazda'))
    model = model_dic[model_inp] 

elif brand_inp == 'Mitsubishi':
    model_inp = col1.selectbox('Enter the model for Volkswagen', options=find_model('Mitsubishi'))
    model = model_dic[model_inp] 
    
elif brand_inp == 'Mitsuoka':
    model_inp = col1.selectbox('Enter the model for Toyota', options=find_model('Mitsuoka'))
    model = model_dic[model_inp] 

elif brand_inp == 'Nissan':
    model_inp = col1.selectbox('Enter the model for BWM', options=find_model('Nissan'))
    model = model_dic[model_inp] 
    
elif brand_inp == 'Subaru':
    model_inp = col1.selectbox('Enter the model for Mercedes Benz', options=find_model('Subaru'))
    model = model_dic[model_inp] 

elif brand_inp == 'Suzuki':
    model_inp = col1.selectbox('Enter the model for Volkswagen', options=find_model('Suzuki'))
    model = model_dic[model_inp] 
...
inp_array = np.array([[brand,model,doors,capac,length,width,height,weight,year,dpclass,wb,mp,displacement,bore,stroke,fte]])
